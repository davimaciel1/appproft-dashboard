...
// Rota pública para métricas agregadas (SEM autenticação)
router.get('/aggregated-metrics', async (req, res) => {
  try {
    ...
    console.log('🔓 PUBLIC Aggregated Metrics Request:', { aggregationType, granularity, startDate, endDate, marketplace, asinLevel });
    ...
    console.error('Error fetching PUBLIC aggregated metrics:', error);
    res.status(500).json({ 
      error: 'Error fetching aggregated metrics', 
      details: error.message,
      note: '🔓 Esta é uma rota pública para testes - sem autenticação necessária'
    });
  }
});
